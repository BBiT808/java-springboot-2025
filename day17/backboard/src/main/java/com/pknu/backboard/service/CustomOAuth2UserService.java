...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    // CustomOAuth2UserService(MemberRepository memberRepository) {
    // this.memberRepository = memberRepository;
    // }

    @Override // Ctrl + . or Ctrl + space 눌러서 추가해주기
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        OAuth2User oAuth2User = super.loadUser(userRequest);
        System.out.println(oAuth2User.getAttributes()); // JSON 형태의 데이터 출력!

        String registationId = userRequest.getClientRegistration().getRegistrationId(); // PROVIDER naver/google/kakao
        OAuth2Response oAuth2Response = null;

        if (registationId.equals("naver")) {
            oAuth2Response = new NaverResponse(oAuth2User.getAttributes());
        } else if (registationId.equals("google")) {
        ...
        if (opMember.isEmpty()) { // 한번도 들어오지 않은 소셜 사용자
            ...
            member.setProvider(registationId);
            ...
            memberDto.setProvider(registationId);
            ...
            existMember.setProvider(registationId);
