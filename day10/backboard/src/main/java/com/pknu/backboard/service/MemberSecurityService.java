...
public class MemberSecurityService implements UserDetailsService {
    ...
    // 스프링시큐리티와 직접 만든 회원정보 테이블을 매핑하는 작업 구현
    @Override
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        Member member = opMember.get(); // 실제 멤버를 가져 옴 !!

        List<GrantedAuthority> authorities = new ArrayList<>();
        if ("admin".equals(username)) {
            authorities.add(new SimpleGrantedAuthority(MemberRole.ADMIN.getValue())); // ROLE_ADMIN 권한 부여
        } else {
            authorities.add(new SimpleGrantedAuthority(MemberRole.USER.getValue())); // ROLE_USER 권한 부여
        }

        // Member 를 직접 쓰는게 아니고, 스프링 시큐리티의 User라는 객체로 로그인, 로그아웃 을 대체
